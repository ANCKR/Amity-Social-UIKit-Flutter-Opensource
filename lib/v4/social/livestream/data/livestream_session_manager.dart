...
class LivestreamSessionManager {
  // Singleton instance
  static final LivestreamSessionManager _instance = LivestreamSessionManager._internal();
  ...
  Future<void> initializeSession() async {
    ...
    try {
      ...
      final accessToken = await _apiClient.createSessionForToken(userId, authToken);
      if (accessToken == null) {
  ...
  Future<String?> getThumbnailUrl(
    ...
    if (!forceRefresh && _thumbnailCache.containsKey(fileId)) {
      ...
      // Return cached if still fresh
      if (DateTime.now().difference(cached.fetchedAt) < _thumbnailCacheDuration) {
